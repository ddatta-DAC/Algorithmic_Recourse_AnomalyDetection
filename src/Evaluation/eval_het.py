...
from Metrics import heterogeneity
LOGGER = None
DIR = None
cf_results_dir = None
model = None
...
def setup_config(_dir_, _model_):
    ...
    base_path = './' 
    LOG_FILE = os.path.join(base_path,'log_results_heterogeneity_{}.txt'.format(_model_))
...
def main():
    ...
    global model
    print('Eval Heterogenity', DIR, model)
    def aux(eval_obj,file):
        ...
        h = eval_obj.eval_het(
                anomaly_record.iloc[0],
                cf_df
            )  
        return h
    
    eval_obj = heterogeneity.heterogeneity_evaluator(_dir_)
    ...
    for file in files:
        r = aux(eval_obj,file)
        res.append(r)
    # res = Parallel(n_jobs=MP.cpu_count(), prefer="threads")(delayed(aux)(eval_obj,file) for file in files)
    LOGGER.info('{}, {} , Hetrogeneity , mean _ {:.4f}, std _ {:.4f}'.format(model, DIR, np.mean(res), np.std(res)))
    return    
